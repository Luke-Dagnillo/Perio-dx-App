...
#         content.add_widget(Label(text=f'Antigen: {antigen}', size_hint_y=None, height=30, halign='center'))
#         content.add_widget(Label(text=f'Result: {result} Concentration', size_hint_y=None, height=30, halign='center'))
#         content.add_widget(Label(text=f'Date: {date}', size_hint_y=None, height=30, halign='center'))
...
class TestResultDetail(ModalView):
    def __init__(self, antigen, result, date, **kwargs):
        ...
        # Close button
        close_button = Button(text='Close', size_hint_y=None, height=40)
        close_button.bind(on_press=lambda *args: self.dismiss())
        content.add_widget(close_button)

        self.add_widget(content)

        # Bind size changes to update text_size dynamically
        antigen_label.bind(size=lambda *x: setattr(antigen_label, 'text_size', antigen_label.size))
        result_label.bind(size=lambda *x: setattr(result_label, 'text_size', result_label.size))
        date_label.bind(size=lambda *x: setattr(date_label, 'text_size', date_label.size))
